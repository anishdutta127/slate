...
- **2026-05-20:** Neon provisioned in `aws-ap-southeast-1` (Singapore). Free tier doesn't expose Mumbai region. Added latency cost ~60ms per query, invisible at our scale due to ISR caching on profiles and small dashboard queries. Migration to Mumbai region requires Neon paid plan; revisit when MAU > 10K or paid plan adopted for other reasons.
- **2026-05-21:** CD view gets one Devanagari touch in the footer wordmark (`Made on Slate · स्लेट`). Reason: the CD is also Indian; the accent signals belonging without compromising the information density of the page. No other Devanagari on the CD; the rest of the page stays pure English/mono.
