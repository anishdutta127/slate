...
**Primary user — the fresher.** 19–28, in Mumbai (most often in Andheri / Versova / Aram Nagar / Goregaon). Often a year or two into the grind. Has a handful of ad-film credits or nothing at all. Currently sends a messy Drive link or a paragraph of YouTube URLs on WhatsApp when applying. No website. Maybe an Instagram. Burned ₹25K on a "portfolio shoot" that delivered jpegs. Tried Talentrack, got lost in 7 lakh profiles. Lives off auditions in person at Aram Nagar.

**Secondary user — the casting professional (CD, casting coordinator, assistant CD).** Sees 200–500 submissions per role on WhatsApp. Scans on mobile in two seconds. Needs: name, age range, height, languages, current city, one photo, one clip, contact. If they don't see those in two seconds, they swipe to the next.
...
- A pretty portfolio for the actor's ego alone — every design choice serves the CD's two-second scan too
...
- **Voice:** Warm, cinematic, slightly Bollywood, slightly underground film festival. Not corporate. Not LinkedIn. Never says "users" — says "actors" or "members". Never says "platform" — says "Slate" or "the club".
...
- **Typography:**
  - Display: a heavy serif with character — `Fraunces` (variable, free on Google Fonts) for big actor names and headlines
...
- **Texture:** Subtle film grain overlay, ~2–3% opacity, fixed position, on dark sections only
- **Motion:** Slow, confident, cinematic. No bouncy springs. Use `Motion` (formerly Framer Motion) on React. Page load = staggered reveal of hero photo → name → tagline → stats. Hover on photos = subtle tilt + warm vignette. Avoid parallax-for-its-own-sake.
...
- **Cinematic view (default):** full-bleed hero photo, name in Fraunces display, one-line tagline, then vertical scroll: showreel → headshots → stats card → training/credits → contact CTA. Subtle motion. Dark mode default.
...
- "Send Profile" button → enter phone number → pick from 4 message presets (Ad film fresher / OTT / Theatre / Custom) → optionally add role / project name → opens WhatsApp via `wa.me/<number>?text=<encoded>` with prefilled message + the link including `?cd=1&ref=<send_id>`.
...
- **Tracking:** every generated link has a `ref` id. We log open events server-side via a redirect or pixel. The actor sees, in their dashboard, a feed: "Opened by [masked number] · 2h ago · watched 18s of reel". One line per send. No funnels, no graphs.
...
- Weekly Sunday meetup in Aram Nagar — free, we host, scene work + cold reads.
...
3. **Your reel.** Paste a YouTube / Drive / Instagram link (we embed cleanly) OR upload a 30s intro video OR record in-browser using the device camera with on-screen prompts ("Say your name. Where you're from. One role you'd love to play."). 60% of freshers have no reel — the in-browser record is the unlock.
...
5. **Ready check.** Live preview of the profile page on a phone frame. "Send Profile" CTA. If the reel is missing, a soft nudge: "Profile is live. Add a reel anytime to make it 10× stronger." Then: "Want to join the Mumbai club?" → link to /club.
...
| WhatsApp send | `https://wa.me/<number>?text=<encoded>` — client opens user's own WhatsApp. **No business API, no template approvals, no cost.** | Our user sends, not us. |
...
GET  /[slug]                        # PUBLIC PROFILE — server component, cached
...
|---|---|---|
| **M0 — Repo + design system** | Tailwind tokens, Fraunces + body font loaded, color vars, base layout, dark theme, grain overlay | Style guide page renders |
| **M1 — Ashish's page, hand-built** | `/ashish` as a static page (no DB). Full visual treatment, both render modes. | Page is shareable, OG image renders correctly on WhatsApp |
| **M2 — Marketing landing + manifesto** | `/`, `/manifesto`, `/club`. The before/after section (messy WhatsApp vs Slate). | Lighthouse 95+ on mobile |
| **M3 — Auth + onboarding** | Phone OTP, 5-step onboard, profile saved to DB. New users get a real `/[slug]`. | A new user can sign up and have a live profile in 15 min |
| **M4 — The Send + tracking** | Dashboard, send flow, ref redirect, opens feed | Sending a profile produces a beautiful WhatsApp preview, opens are tracked |
| **M5 — Club applications** | Form on `/club`, admin view to approve | First 20 club members onboarded manually |
| **M6 — Launch day** | Aram Nagar Sunday event. Press kit. Founder-first profiles seeded. | We have a public moment |

Aim: M0–M2 in week 1. M3–M4 in week 2. M5–M6 in week 3.
...
- Status: working ad-film actor (not a fresher — he is the aspiration for our fresher audience)
- Credits (all real, all should appear on his page):
  - Honda — TVC — https://www.instagram.com/reel/DXGdDOHjA6_/
  - Cipla — TVC — https://www.instagram.com/p/DLPad3QK2s1/
  - Zepto — TVC — https://www.facebook.com/share/v/1AYq8JJYEJ/
  - Nilkamal Furniture — Digital — https://youtu.be/tnVDz4HvhbE
  - Smotect — Digital — https://www.instagram.com/reel/DMaZV5ht1Am/
  - Rings & I — Digital — https://www.facebook.com/share/r/1HtfW5h7CB/ + https://www.facebook.com/share/r/1EEdLQR3ZB/
